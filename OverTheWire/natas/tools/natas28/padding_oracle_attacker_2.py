...
class ChosenPlaintext(object):
    ...
    def blocks(self, c, bs=None):
        if bs is None:
            bs = self.block_size
        return [c[i * bs:(i + 1) * bs] for i in range(int(len(c) / bs))]
    ...
    def run(self):
        ...
        known_block = int((self.plaintext_offset + plength) / self.block_size) - 1
        ...
        while len(self.plaintext) < (clen - self.plaintext_offset):
            ...
            p = ('b' * (plength - 1 - len(self.plaintext)))
            ...
            for i in range(0, 255):
                p = ('b' * (plength - 1 - len(self.plaintext))) + self.plaintext + chr(i)
                c = self.__get_ciphertext(p)
...
class Client(ChosenPlaintext):
    ...
    def ciphertext(self, plaintext):
        print(f"[*] Trying plaintext: {plaintext.encode()}")
        ...
        data = base64.b64decode(unquote(resp.url.split("query=")[1]))
        print("[*] Got ciphertext: %s" % base64.b64decode(unquote(resp.url.split("query=")[1])))
